...
public class LazySegmentTree {
    final int MAX = 1000;        // Max graph.tree size
    int tree[] = new int[MAX];  // To store segment graph.tree
    int lazy[] = new int[MAX];  // To store pending updates

    /**
     * si -> index of current node in segment graph.tree
    ...
    void updateRangeUtil(int si, int ss, int se, int us,
        ...
        // graph.tree, then there are some pending updates. So we need
        ...
        if (ss >= us && se <= ue) {
            ...
            if (ss != se) {
                ...
                // rather than updating the segment graph.tree itself
        ...
        // And use the result of children calls to updateHighestHeightBtwLR this
        // node
        tree[si] = tree[si * 2 + 1] + tree[si * 2 + 2];
    }

    // Function to updateHighestHeightBtwLR a range of values in segment
    // graph.tree
     ...
     * this function.
     * si --> Index of current node in the segment graph.tree.
    ...
    int getSumUtil(int ss, int se, int qs, int qe, int si) {
        // If lazy flag is set for current node of segment graph.tree,
        ...
        // was for queryHighestHeightBtwLR in our previous post)
    ...
    void constructSTUtil(int arr[], int ss, int se, int si) {
        ...
        /* If there is one element in array, store it in
         current node of segment graph.tree and return */
        if (ss == se) {
            tree[si] = arr[ss];
            return;
        }
    ...
    /* Function to construct segment graph.tree from given array.
       This function allocates memory for segment graph.tree and
