 ...
 * 3、Create dp[] table and traceback[] table of the same size to
...
public class BoxStacking {
    ...
    public int MaxHeight(Dimension[] input) {
        ...
        Arrays.sort(allInput, (a, b) -> (a.width * a.length - b.width * b.length));
    ...
    public Dimension[] createAllDimensions(Dimension[] input, Dimension[] allRotationInput) {
        ...
        for (int i = 0; i < input.length; i++) {
            ...
            allRotationInput[index++] = Dimension.createDimension(input[i].width, input[i].height, input[i].length);
...
class Dimension implements Comparable<Dimension> {
    ...
    static Dimension createDimension(int height, int length, int width) {
