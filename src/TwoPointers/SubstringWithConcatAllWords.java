...
public class SubstringWithConcatAllWords {
	...
	// seen[someString] should be <= maps[someString]; otherwise, we need to advance the left pointer by one more word (wl steps) 
		...
		for (int i=0; i<wl; i++) {
			...
			for (int left = i, right = i; right < N - wl + 1; right += wl) {
				...
				if (maps.containsKey(str)) {
					...
					if (maps.get(str) > seen.get(str)) {
						count ++;
					} 
					else {
						// move left pointer forward by one more word
						while (maps.get(str) < seen.get(str)) {
							String temp = s.substring(left, left + wl);
							seen.put(temp, seen.get(temp) - 1);
	
							// move to the right by one word, if temp is a word in words
							// we get to lose one matched word, so count = count - 1;
							if (maps.get(temp) > seen.get(temp)) {
								count--;
							}
				...
				else {// the str does not exist in the maps reset all vars; left pointer goes to the position of right pointer
					count = 0;
					left = right + wl;
					seen.clear();
				}
