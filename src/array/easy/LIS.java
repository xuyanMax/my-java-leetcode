...
public class LIS {
    ...
    public int lengthOfLIS_dfs_with_memo(int[] nums) {
        ...
        return dfs_memo(nums, -1, memo, 0);
    }
    public int dfs_memo(int[]nums, int preindex, int[][] memo, int currpos){
        ...
        if (preindex < 0 || nums[currpos] > nums[preindex]) {
            taken = 1 + dfs_memo(nums, currpos, memo, currpos + 1);
        }
        int nottaken = dfs_memo(nums, preindex, memo, currpos + 1);
