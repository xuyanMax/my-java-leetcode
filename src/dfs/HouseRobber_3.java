...
public class HouseRobber_3 {
    ...
    public int rob_naive(TreeNode root) {
        ...
        if (root.right != null)
            val += rob_naive(root.right.left) + rob_naive(root.right.right);

        return Math.max(val, rob_naive(root.left) + rob_naive(root.right));
    ...
    // GREEDY 贪婪算法
    // 这回我们重新定义rob(TreeNode root)，返回值为一维两个元素的数组，以此保留从该root开始，包含该节点与不包含该节点的最大收益分别是多少
    //
    ...
    public int[] rob_greedy(TreeNode root) {
        ...
        // ret[0]:不包括root值
        // ret[1]:包括root值
        // 不包含root：包含左子节点／不包含的较大值 + 包含右子节点／不高喊的较大值
