...
public class KnapsackComplete {
    ...
    // dynamic[i][j] = max(dynamic[i-1][j], dynamic[i-1][j-k*wgt[i]] + k*val[i]) k in [0:W/wgt[i]]
        ...
        for (int i=1; i<=n; i++) {//item i
            for (int j=0; j<=W; j++) {// weight/cost of item i
                ...
                for (int k=0; k<=cnt; k++) //对一个item 最多取 W/wgt[i]向下取整个
                    dp[i][j] = Math.max(dp[i][j], dp[i-1][j - k* wgt[i-1]] + k*val[i-1]);// k = 0， = dynamic[i-1][j]
            }
        }
        return dp[n][W];
    }
    ...
    //这是最优解 O(NW)
    // dynamic[i][j] = max(dynamic[i-1][j], dynamic[i][j-wgt[i]] + val[i]);
    // 循环方式：顺序循环（O(n)空间解）
    // 完全背包，不限次数，与0/1背包
    // dynamic[i][j] = max(dynamic[i-1][j], dynamic[i][j - wgt[i]] + val[i])
    // 这里的dp[i][j - wgt[i] + val[i]]而不是dp[i-1][...]
    // 原因是，0/1背包中，dynamic[i][j]的状态是由dp[i-1][j-wgt[i]]推出来的，即为了保证每个物品只选一次，保证在"考虑选择物品i"策略时，
    // 依据绝无可能选入物品i的子结果dp[i-1][j-wgt[i]]。然而，完全背包，不限制个数，则"考虑选择物品i"这种策略时，需要考虑"可能入选了物品i"的字结果
    // dynamic[i][j - wgt[i]]，因此必须采用j = wgt[i]:W的顺序循环
