...
public class WeightedJobScheduling {
	 ...
	 * 	
	 * 		dynamic[i] = max(dynamic[i], dynamic[j]+jobs[i].profit) where j in [0, i-1];
	 * 
	 * 
	 * 1. First, sort jobs by early finish time
	 * 
	 * 2. Initially dynamic[i] = jobs[i].profit: meaning the max profit until the i-th job
	 * 
	 * 3. Update the max profit for dynamic[i] by scanning the non-overlapping jobs before i-th job
	 * 	  by the formula dynamic[i] = max(dynamic[i], jobs[i].profit + dynamic[j]) where j in [0,i-1]
	 * 
	 * 
	 * @param jobs
	 * @return
	...
	public int MaxProfit(Job[] jobs){
		...
		for (int i=1; i<dp.length; i++) {
			dp[i] = jobs[i].profit;//Math.max(dynamic[i-1], jobs[i].profit);
			for (int j=i-1;j>=0;j--){
