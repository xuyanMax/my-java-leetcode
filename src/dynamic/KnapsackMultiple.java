...
//    public void knapsackMultiPack(int[] wgt, int[]val, int[] count, int W, int n, int i, int[] dynamic){
//        if (wgt[i-1] * count[i-1] <= W)//完全背包
//            knapsackComplete(wgt, val, count, W, n, i);
//        else {
//            //0/1背包
...
//            while (k < count[i-1]) {
//                for (int j=W; j>=k*wgt[i-1]; j--)
//                    dynamic[j] = Math.max(dynamic[j], dynamic[j - k*wgt[i-1]] + k*val[i-1]);
//                count[i-1] -= k;
//                k <<= 1;
//            }
//            // 0/1背包
//            // 计算剩余的 x 件i物品
//            for (int j = W; j>=count[i-1]*wgt[i-1]; j--)
//                dynamic[j] = Math.max(dynamic[j], dynamic[j - count[i-1]*wgt[i-1]] + count[i-1]*val[i-1]);
//        }
//    }
