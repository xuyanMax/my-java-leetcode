...
public class BestTimeToBuyAndSellStock4 {
    ...
    // dynamic[k][j] = Math.max(dynamic[k][j-1], dynamic[k-1][m]+Math.max(prices[j] - prices[m]) where i in 0:j-1;
    ...
    // dynamic[i][k][0] = max(dynamic[i-1][k][0], dynamic[i-1][k-1][1] + prices[i])
    // dynamic[i][k][1] = max(dynamic[i-1][k][1], dynamic[i-1][k-1][0] - prices[i])
