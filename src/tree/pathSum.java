...
public class pathSum {
	...
	static void pathSum2Util(List<List<Integer>> paths, List<Integer> path , AvlNode root, int sum) {
		...
		if (root.left == null & root.right == null && sum == root.data) 
			...
			pathSum2Util(paths, path, root.left, sum-root.data);
			pathSum2Util(paths, path, root.right, sum-root.data);
		}
	...
	static void allPathSumUtil(AvlNode root, List<Integer> pathSums, int sum) {
		...
		if (root.left!=null)
			allPathSumUtil(root.left, pathSums, sum + root.left.data);
		
		if (root.right!=null)
			allPathSumUtil(root.right, pathSums, sum + root.right.data);;
